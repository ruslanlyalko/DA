            ...
            lock (LockReader)
            {
                ...
                if (reader != null)
                {
                    while (reader.Read())
                    {
                        ...
                        user.Id = reader.GetInt32(16);
                        user.Name = reader.GetString(17);
                        user.Password = reader.GetString(18);
                        user.FullName = reader.GetString(19);
                        user.Email = reader.GetString(20);
                        user.Phone = reader.GetString(21);
                        user.IpAdress = reader.GetString(22);
                        user.Blocked = reader.GetBoolean(23);
                        user.AllowDataNet = reader.GetBoolean(24);
                        user.AllowTickNet = reader.GetBoolean(25);
                        user.AllowLocalDb = reader.GetBoolean(26);
                        user.AllowRemoteDb = reader.GetBoolean(27);
                        user.AllowAnyIp = reader.GetBoolean(28);
                        user.AllowMissBars = reader.GetBoolean(29);
                        user.AllowCollectFrCqg = reader.GetBoolean(30);
                        userList.Add(user);
                    }
                    reader.Close();
                }
