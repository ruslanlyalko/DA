    ...
    class  DataCollector
    {
        ...
        private void BarRequest(CQGCEL cel, IEnumerable<string> symbols, int rangeStart, int rangeEnd, int sessionFilter, string historicalPeriod, string continuationType)
        {
            ...
            foreach (string smb in symbols)
            {
                ...
                DataNetClientDataManager.CreateBarsTable(smb, _aTableType);
        ...
        private void AddBar(CQGTimedBar timedBar, string symbol, DateTime runDateTime, string tType, string userName)
        {
            ...
            try
            {
                ...
                DataNetClientDataManager.AddToQueue(sql,1);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        public void TicksAdd(CQGTicks cqg_ticks, CQGError cqgError, string userName)
        {
            ...
            try
            {
                ...
                {
                    DataNetClientDataManager.CreateTickTable(cqg_ticks.Request.Symbol);
                    ...
                    {
                        DataNetClientDataManager.DeleteTicks(cqg_ticks.Request.Symbol, cqg_ticks[0].Timestamp, cqg_ticks[cqg_ticks.Count - 1].Timestamp);
        ...
        private void MissingBarRequest(CQGCEL cel, List<string> symbols, int maxCount, bool isAuto = false)
        {
            ...
            _aSymbolStates.Clear();
            DataNetClientDataManager.CreateMissingBarExceptionTable();
            DataNetClientDataManager.CreateSessionHolidayTimesTable();
            DataNetClientDataManager.CreateFullReportTable();
        ...
        private void StartAsyncGetingSessionsData(CQGCEL cel, IEnumerable<string> symbols)
        {
            foreach (string symbol in symbols)
            {
                ...
                List<DateTime> aResultDateTimes = DataNetClientDataManager.GetAllDateTimes(DataNetClientDataManager.GetTableFromSymbol(symbol));
        ...
        private void StartAsyncCheckingMissedBars(IEnumerable<string> symbols)
        {
            ...
            foreach (string currentSymbol in symbols)
            {
                ...
                DataNetClientDataManager.DelFromReport(currentSymbol);


                var aResultDates = DataNetClientDataManager.GetAllDates(DataNetClientDataManager.GetTableFromSymbol(currentSymbol));
                var aResultDateTimes = DataNetClientDataManager.GetAllDateTimes(DataNetClientDataManager.GetTableFromSymbol(currentSymbol));
                ...
                DateTime refresh = DateTime.Now;
                {
                    ...
                    for (DateTime curDt = aResultDates.First(); curDt <= aResultDates.Last(); curDt = curDt.AddDays(1))
                    {
                        ...
                        if (DataNetClientDataManager.HolidaysContains(DataNetClientDataManager.GetTableFromSymbol(currentSymbol), curDt))
                        {
                        ...
                        DataNetClientDataManager.AddToReport(currentSymbol, curDt, state.ToString(), sTime.DayOfWeek.ToString(), sTime, eTime.DayOfWeek.ToString(), eTime);
                        ...
                        if (state == SessionStates.OpenedHoliday || state == SessionStates.OpenedNormal)
                        {
                            ...
                            foreach (MissedStr item in aMissedList)
                            {
                                ...
                                DataNetClientDataManager.AddToReport(currentSymbol, item.Start, state.ToString(), item.Start.DayOfWeek.ToString(), item.Start, item.End.DayOfWeek.ToString(), item.End);
                            }
                        }
                    }//end:for curDT
                }// end:if                                
                ...
                var aMissedBarsForSymbol = DataNetClientDataManager.GetMissedBarsForSymbol(currentSymbol);
                ...
                foreach (DateTime missedItem in aSmallMissedBarsForSymbol)
                {
                    ...
                    {
                        DataNetClientDataManager.ChangeBarStatusInMissingTableWithOutCommit(currentSymbol, refresh, missedItem);
                    }
                }

                DataNetClientDataManager.CommitQueue();
        ...
        private void StartAsyncCheckingMissedBarsAuto(IEnumerable<string> symbols, int maxCount)
        {
            ...
            foreach (string currentSymbol in symbols)
            {
                ...
                var aResultDateTimes = DataNetClientDataManager.GetAllDateTimes(DataNetClientDataManager.GetTableFromSymbol(currentSymbol), maxCount + 1);

                var aResultDates = DataNetClientDataManager.GetAllDates(DataNetClientDataManager.GetTableFromSymbol(currentSymbol), maxCount + 1);
                if (aResultDateTimes.Count > 0)
                    DataNetClientDataManager.DelFromReport(currentSymbol, aResultDateTimes.First());
...

                {
                    ...
                    var res = DataNetClientDataManager.GetReport(currentSymbol);
        ...
        private IEnumerable<MissedStr> MissedInTable(string smb, List<DateTime> aResultDateTimes, DateTime missDateTimeStart, DateTime missDateTimeEnd, bool dayStartsYesterday, bool skipAddingToDb = false)
        {
            ...
            for (var curTime = startDateTime; curTime < missDateTimeEnd; curTime = curTime.AddMinutes(1))
            {
                ...
                if (!skipAddingToDb)
                    DataNetClientDataManager.AddToMissingTableWithOutCommit(smb, refresh, curTime);
        ...
        public void SessionAdd(CQGSessions sessions, string symbol)
        {
            ...
            try
            {
                foreach (CQGSession session in sessions)
                {
                    ...
                    DataNetClientDataManager.AddToSessionTable(symbol, symbol, session.StartTime, session.EndTime, "Open",
        ...
        public void HolidaysAdd(CQGSessionsCollection sessions, string symbol)
        {
            ...
            try
            {
                foreach (CQGSessions session in sessions)
                {
                    ...
                    {
                        DataNetClientDataManager.AddToSessionTable(symbol, symbol, holiday.HolidayDate, holiday.HolidayDate, "Holiday", "", false, false, 0, DateTime.Now);
                    }
